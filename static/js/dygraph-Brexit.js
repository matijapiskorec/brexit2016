...
DygraphCanvasRenderer._drawStyledLine = function(e,
  ...
  // DEJAN:
  if (e.seriesIndex==2 && e.setName=="Average") {
     var pozicija=0;
     for (var i = 0; i < e.points.length; i++) {
       if (!isNaN(e.allSeriesPoints[2][i].yval)) {
         pozicija=e.allSeriesPoints[2][i].xval;
         };
    };
    if (pozicija<50) {color='blue';}
    else {color='red';}
  }
...
Dygraph.addAnnotationRule = function() {
  ...
  var rule = "border: 0px solid black; " +
...
legend.generateLegendHTML = function(g, x, sel_points, oneEmWidth, row) {
  ...
  for (i = 0; i < sel_points.length; i++) {
    ...
    html += "<span" + cls + ">" + " <b><span style='color: " + series.color + ";'>" +
        //DEJAN: escapeHTML(pt.name) + "</span></b>:&#160;" + yval + "</span>";
